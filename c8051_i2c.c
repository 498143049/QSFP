...
{
	RX_I2C_SDA = 1; //释放I2C总线的数据线
	RX_I2C_SCL = 1; //释放I2C总线的时钟线
}
/************************************************
...
void RX_I2C_Start()
{
	...
	RX_I2C_SCL = 1;   //
	I2C_Delay();  //延迟5个机器周期
	RX_I2C_SDA = 0;   //SCL高，SDA下降沿，产生起始信号
	I2C_Delay();  //
	RX_I2C_SCL = 0;   //
}
...
void RX_I2C_Stop()
{
	...
	RX_I2C_SCL = 1;
	I2C_Delay(); //延时5个机器周期
	RX_I2C_SDA = 1;  //SCL为高，SDA上升沿，产生停止信号
	I2C_Delay();
}
/************************************************
*	函数名称：I2C_Ack
...
*   作    者：
*************************************************/
void RX_I2C_Write8Bit(unsigned char DData)
{
	unsigned char i;
	unsigned char temp;
	temp = DData;
	for(i=0; i<8; i++)
	{
		RX_I2C_SDA = temp & 0x80; //根据I2C协议从最高位开始发送
		I2C_Delay();
		RX_I2C_SCL = 1;
		I2C_Delay();
		RX_I2C_SCL = 0;
...
{
	RX_I2C_Start();
	RX_I2C_Write8Bit(Daddr); //写入从机设备地址
	RX_I2C_Ack();
	RX_I2C_Write8Bit(Waddr); //写入数据地址
	RX_I2C_Ack();
	RX_I2C_Write8Bit(Data);   //写入数据
	RX_I2C_Ack();
	RX_I2C_Stop();   //停止发送数据
}
//RX多字节写 
void RX_I2C_BytesWrite(unsigned char Daddr, unsigned char Waddr, unsigned char *DataArray,unsigned char num)
{
	...
	while(Temp>0)
	{
		RX_I2C_Write8Bit(*DataArray++);   //写入数据
		RX_I2C_Ack();
		Temp--;
	}
	RX_I2C_Stop();   //停止发送数据
...
unsigned char RX_I2C_SingleByteRead(unsigned char Daddr, unsigned char Waddr)
{
	unsigned char ReadData;
	RX_I2C_Start();
	RX_I2C_Write8Bit(Daddr); //写入设备地址
	RX_I2C_Ack();
	RX_I2C_Write8Bit(Waddr); //写入数据操作地址
	RX_I2C_Ack();
	RX_I2C_Stop();

	RX_I2C_Start();
	RX_I2C_Write8Bit(Daddr+1);    //写入设备地址
	RX_I2C_Ack();
	ReadData = RX_I2C_Read8Bit(); //读出I2C中的数据
	RX_I2C_Stop();
