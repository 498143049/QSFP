...
#define Current  0
#define ReadNext 1 
uint8_t xdata _currentAddress=0; //recode the Inter_Address 
uint8_t xdata  ReciverAddress=0;
uint8_t xdata  GetValueNum=1;
uint8_t xdata  OperateStatus=0;
bit  _READ_TYPE=Current;
bit	 _QSFP_Status=READ;       //1
extern  uint8_t xdata SMBUSFIFOReciver[10];
extern  uint8_t xdata ReiciverbufferSize;
extern uint8_t xdata  JourBuffer[10];
// varify the Address is equal to the QSFP
//read operate request being  dealing while the reciver the request  
//write operate request  being dealing while the Stop interrput 
